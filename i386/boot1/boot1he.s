...
;
; Partition Boot Loader: boot1h extended sectors
;
; This program is designed to reside between sectors 40-47 of a block device.
; It expects that the partition boot loader left the drive number in DL
; and a pointer to the partition entry in SI. Starting sector of this loader
; should be passed in ECX.
; 
; This version requires a BIOS with EBIOS (LBA) support.
;
; This code is written for the NASM assembler.
;   nasm boot1he.s -o boot1he

;
; This version of boot1he tries to find a stage2 boot file in the root folder.
;
;
; Written by zef @ 2008-04-17,
; updated with large boot file loading support @ 2008-10-27
;
...
startReloc:
    ...
    ;
    ; Loading remaining sectors of boot1h extended code.
    ;
    ...
    mov     al, (kBoot1ExtSize - kSectorBytes) / kSectorBytes
	inc		ecx								; ECX = next sector to boot1h extended code starting sector
    mov     edx, kBoot1ExtAddr
    call    readLBA
...
%if VERBOSE
log_title_str		db		CR, LF, 'boot1ext: ', NULL
...
pad_table_and_sig:
	times			(kSectorBytes - 6)-($-$$) db 0
	dd				kBoot1ExtSignature
	dw				kBootSignature
...
;--------------------------------------------------------------------------
; readExtent - read extents from a HFS+ file (single extent support)
;
; Arguments:
;  EAX = Catalog File ID
;  	BX = read size in sectors
...
readExtent:
	...
	mov		[bp + BTree.searchExtentKey + HFSPlusExtentKey.fileID], eax
	push	edx
	mov		eax, ecx						; EAX = relative offset in sectors	
	mov		edx, [di]
	bswap	edx
	mov		[bp + BTree.searchExtentKey + HFSPlusExtentKey.startBlock], edx
	bswap	edx
	call	blockToSector					; ECX = starting block of current extent
	add		ecx, eax						; ECX = relative sector within current partition
	pop		edx
	mov		al, bl
	call	readLBA
	ret
...
searchCatKeyName	dw		'b', 'o', 'o', 't'			; must be upper case
searchCatKeyNameLen	EQU		($ - searchCatKeyName) / 2

;--------------------------------------------------------------------------
; Pad the rest of the 4096 byte sized loader with zeroes. The last
; two bytes is the mandatory boot sector signature.
;
pad_sectors:
	times			kBoot1ExtSize-($-$$) db 0
